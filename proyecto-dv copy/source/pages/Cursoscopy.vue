...
export default {
  ...
  computed: {
    productosFiltrados() {
      return this.productos.filter(producto => producto.habilitado);
    },
  },
};
</script>
  ...
  <section>
    ...
    <div class="max-w-7xl pb-20 mx-auto -mt-48">
        <ul class="grid grid-cols-1 gap-16 lg:grid-cols-3 sm:gap-8">
          <li v-for="producto in productosFiltrados" :key="producto.id" class="border-0 rounded-none shadow-none card sm:shadow-md sm:rounded-lg">
