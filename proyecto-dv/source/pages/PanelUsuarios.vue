                ...
                <li v-for="user in users" :key="user.id" class="flex justify-between gap-x-6 py-5">
                    <div class="flex min-w-0 gap-x-4">
                        <div class="flex items-center justify-center w-12 h-12 mb-4 bg-gray-50 rounded-full">
                            <ProfileImage :src="user.photoUrl" class="h-12 w-12 flex-none rounded-full bg-gray-50"/>
                        </div>
                        <div class="min-w-0 flex-auto">
                            <p class="text-sm font-semibold leading-6 text-gray-900">{{ user.displayName || "Sin nombre de usuario"}}</p>
